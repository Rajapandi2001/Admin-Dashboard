 <!doctype html>
<html lang="en">

<?php require_once("includes/head.php");  ?>
<?php require_once("includes/media-helper.php");  ?>

  <body class="font-body text-base font-light lg:text-md text-secondary antialiased">
        <a href="#content" role="button" aria-label="Skip to content"></a>
	 <?php require_once("includes/header.php");  ?>  
	 
	 <main class="" id="content">
              <section data-block="hero" class="h-screen" data-sal="fade">
  

  <picture>
         <source
        srcset=
          "<?= getMediaPath('mission_hero_42') ?>" data-media-key="mission_hero_42"
        media="(min-width: 1024px)"
        type="image/webp">
      <source
        srcset=
          "<?= getMediaPath('mission_hero_42') ?>" data-media-key="mission_hero_42"
        media="(min-width: 768px)"
        type="image/webp">
      <source
        srcset=
          "<?= getMediaPath('mission_hero_42') ?>" data-media-key="mission_hero_42"
        type="image/webp">
        <source
      srcset=
        "<?= getMediaPath('mission_hero_42') ?>" data-media-key="mission_hero_42"
      media="(min-width: 1024px)">
    <source
      srcset=
        "<?= getMediaPath('mission_hero_42') ?>" data-media-key="mission_hero_42"
      media="(min-width: 768px)">
    <img
      src=
        "<?= getMediaPath('mission_hero_42') ?>" data-media-key="mission_hero_42"
      class="w-full h-full object-cover "
      alt="Pre-prep children jumping off rubber tyres "
      width="562"
      height="945"
      loading="lazy">
</section>
    <article
  id="pageContent"
  class="relative animate-slide-up -top-[var(--travel)] -mb-[var(--travel)]"
  style="--delay: 800ms; --travel: 50vh; --hero-bg-h: 20vw;">
          <span class="block h-[var(--hero-bg-h)] bg-no-repeat -mb-px" style="background-image: url(images/hero-top-three.svg?id=b2be2c9fbfb425d1405c486560808de9);background-size: 100%;"></span>
        <div class="bg-white pt-8 lg:pt-0">
      <div class="container-sm relative mb-12 md:mb-16 lg:-top-12 2xl:-top-24 lg:mb-8 2xl:mb-0 animate-fade-in" style="--delay: 2s;">
        <nav class="mb-8">
  <ul class="flex gap-4 items-center h5">
                  <li><a href="#">About us</a></li>
        <li>
          <i class="block w-2.5 text-primary"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 14">
  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M1.5 12c.52-.37 1.035-.824 1.56-1.357a6.367 6.367 0 0 1 1.6-.836 6.19 6.19 0 0 0 1.546-.785c.45-.32.837-.658 1.158-1.01.313-.357.682-.76 1.136-1.083-.334-.315-.676-.67-1.02-.914l-.966-.69a7.973 7.973 0 0 1-.73-.584 16.705 16.705 0 0 0-1.117-.861C3.38 2.961 2.41 2.333 1.762 2"></path>
</svg>
</i>
        </li>
              <li><a href="#">Our Story, Vision & Mission</a></li>
        <li>

        </li>
  </ul>
</nav>
        <h1 class="h1 text-primary text-balance">Our Story, Vision & Mission</h1>
                  <h2 class="h3 text-primary mt-4">Inspiring Excellence, Shaping Futures</h2>
                          <div class="prose mt-12"><p>We are committed to nurturing well-rounded individuals through quality education, innovation, and personal growth. Our school empowers students to achieve their best and make a positive impact on the world.

</p></div>
              </div>

              <section
  data-block="divide"
  class="overflow-hidden">
  <figure class="min-w-full w-screen-2xl">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1320 20">
  <path fill="none" fill-rule="evenodd" stroke="#DDD" d="M1319.749 17.595c-.737-.65-1.455-1.3-2.192-1.943-2.382-2.384-5.249-2.846-8.361-2.306-9.055-4.97-18.656-3.424-28.19-2.579-13.88-2.911-27.84-2.04-41.806-1.079-3.327.156-6.7-.962-9.982.37-12.21-1.079-24.433-2.151-36.686-.806-.9.188-1.811.383-2.707.578-1.1-.474-2.216-1.4-3.284-1.351-19.386.949-38.816-2.053-58.178.7l-7.25-1.033c-3.886-.169-7.815-1.215-11.615.52-.933.006-1.878.019-2.805.026-12.578-3.352-25.335-.942-38.006-1.4a5.675 5.675 0 0 0-5.783.689l-1.486.078c-1.283-.247-2.578-.487-3.861-.734-.571-.442-1.191-1.325-1.719-1.26-18.527 2.267-36.938-1.9-55.372-1.338-14.709.455-29.417-2.248-44.071.435l-3.782.117c-9.288-3.794-18.92-2.365-28.472-2.157-.872 0-1.756 0-2.634-.006a7.409 7.409 0 0 0-6.028.474c-.694-.019-1.381-.032-2.081-.052a109.752 109.752 0 0 0-30.044-.559c-.951-.149-1.915-.305-2.873-.455-3.383-1.7-6.771-.877-10.16-.032-5.255.3-10.559-1.429-15.783.416a7.6 7.6 0 0 0-6.034.351c-1.412-.052-2.842-.1-4.26-.162-8.772-2.04-17.606-1.709-26.465-.8h-5.857c-10.688-1.2-21.363-2.164-32.057 0h-2.99c-10.663-2.807-21.326-.227-31.99.039-2.346-1.263-4.785-.905-7.238-.464a126.639 126.639 0 0 0-32.014.624c-1.424.052-2.861.1-4.279.143-3.953.11-7.888.214-11.836.318-3.376-1.559-6.642-.832-9.9.455a8.635 8.635 0 0 0-7.5.221c-1.222-.006-2.437-.006-3.653-.013-2.185-2.2-5-1.735-7.557-1.683-8.404.169-16.808.502-25.212 1-14.046.825-28.147-1.247-42.162.988-1.449.026-2.893.05-4.334.071a7.324 7.324 0 0 0-7.342.143c-1.455.117-2.9.24-4.359.357-3.855 1.813-7.839-2.034-11.682.279-.774-.026-1.547-.045-2.327-.065-1.7-1.949-3.4-2.579-5.108-.006-.657.078-1.308.156-1.964.24-4.484-3.441-9.291-1.341-14.005-.941a3.877 3.877 0 0 0-4.027.24l-9.1.234a6.532 6.532 0 0 0-5.66-1.1c-1.541-1.092-3.045-.682-4.537.149a6.056 6.056 0 0 0-5.562.286 4.05 4.05 0 0 0-4.549.11 75.885 75.885 0 0 0-20.228.559c-19.45-2.414-38.93-1.205-58.408-.406-1.921 0-3.843-.006-5.771-.006-3-1.2-5.906-.429-8.809.461-1.909.006-3.818.006-5.728.013a1.913 1.913 0 0 0-3.063.1c-1.393.032-2.8.065-4.193.1-9.755-2.293-19.571-1.319-29.387-.637l-4.119.117a6.566 6.566 0 0 0-6.028.481l-7.373-.253c-3.812-2.125-7.631-1.54-11.431.032-2.431-.039-4.862-.071-7.293-.11-3.468-.539-6.931-1.033-10.375.143-1.682-.942-3.315-.409-4.954.169-4.985-3.164-10.3-.871-15.47-1.072-.97-.143-1.94-.292-2.91-.435-3.818-2.228-7.8-.565-11.7-.526l-8.742-.1c-.933-1.39-1.811-1.338-2.658.084-3.413.013-6.876-.422-10.166.981-8.22.273-16.649-2.04-24.568 2.157-1.166.11-2.333.214-3.5.318-4.168-2.69-8.57-.91-12.892-.767l-2.769.156c-1.179-.793-2.155-.682-2.861.708-.945.247-1.9.487-2.848.734-8.146-.741-16.262-1.377-23.911 2.7a122 122 0 0 0-22.9.24c-1.08-.442-2.161-1.28-3.235-1.267-13.462.175-26.937.305-39.964 4.678-2.977-.2-5.961-.63-8.834.65-8.815-1.793-17.655-1.26-26.5-.331a8.714 8.714 0 0 0-7.017.273c-3.475-.214-6.968-.858-10.393.383-5.279-.448-10.583-1.241-15.8.6-2.726-.533-5.046.9-7.5 1.709-4.819-1.91-9.669-1.28-14.525-.188-.915-1.117-1.829-1.286-2.769-.045-4.3.78-8.809-.572-12.99 1.423l-10.454.39a14.728 14.728 0 0 0-9.914.39 2 2 0 0 0-1.663-.25 2.065 2.065 0 0 0-1.28 1.108"></path>
</svg>

  </figure>
</section>
              
<section
data-block="text"
class="relative bg-white pt-16 lg:pt-32 mb-16 lg:mb-32 ">
  <div class="container-sm space-y-12">
    <div class="gap-y-4 text-center flex flex-col w-full items-center ">
            <h2 class="h2 text-line grow-0">Our Stroy</h2>
        </div>
    <div class="prose">
	<p>Midas Touch School of Excellence has proudly dedicated several years to the field of education, consistently striving to nurture young minds with care and commitment. From the very beginning, our goal has been more than just academic excellence—we aim to inspire and empower students to become lifelong learners, critical thinkers, and responsible members of a rapidly evolving society. With the academic partnership of Next Education India Pvt. Ltd., we continue to set high standards in education, providing a holistic and globally relevant learning experience.</p>
		<p>Our school offers a platform for every student to think creatively, express confidently, and showcase their unique talents. We believe in identifying, nurturing, and encouraging each child’s abilities, helping them reach their highest potential. Backed by a rich history of achievements, Midas Touch School of Excellence moves forward with confidence, pride, and an unwavering enthusiasm for excellence.</p>
       </div>
      </div>
  <div class="clear-both"></div>
</section>              <section
  data-block="divide"
  class="overflow-hidden">
  <figure class="min-w-full w-screen-2xl">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1320 20">
  <path fill="none" fill-rule="evenodd" stroke="#DDD" d="M1319.749 17.595c-.737-.65-1.455-1.3-2.192-1.943-2.382-2.384-5.249-2.846-8.361-2.306-9.055-4.97-18.656-3.424-28.19-2.579-13.88-2.911-27.84-2.04-41.806-1.079-3.327.156-6.7-.962-9.982.37-12.21-1.079-24.433-2.151-36.686-.806-.9.188-1.811.383-2.707.578-1.1-.474-2.216-1.4-3.284-1.351-19.386.949-38.816-2.053-58.178.7l-7.25-1.033c-3.886-.169-7.815-1.215-11.615.52-.933.006-1.878.019-2.805.026-12.578-3.352-25.335-.942-38.006-1.4a5.675 5.675 0 0 0-5.783.689l-1.486.078c-1.283-.247-2.578-.487-3.861-.734-.571-.442-1.191-1.325-1.719-1.26-18.527 2.267-36.938-1.9-55.372-1.338-14.709.455-29.417-2.248-44.071.435l-3.782.117c-9.288-3.794-18.92-2.365-28.472-2.157-.872 0-1.756 0-2.634-.006a7.409 7.409 0 0 0-6.028.474c-.694-.019-1.381-.032-2.081-.052a109.752 109.752 0 0 0-30.044-.559c-.951-.149-1.915-.305-2.873-.455-3.383-1.7-6.771-.877-10.16-.032-5.255.3-10.559-1.429-15.783.416a7.6 7.6 0 0 0-6.034.351c-1.412-.052-2.842-.1-4.26-.162-8.772-2.04-17.606-1.709-26.465-.8h-5.857c-10.688-1.2-21.363-2.164-32.057 0h-2.99c-10.663-2.807-21.326-.227-31.99.039-2.346-1.263-4.785-.905-7.238-.464a126.639 126.639 0 0 0-32.014.624c-1.424.052-2.861.1-4.279.143-3.953.11-7.888.214-11.836.318-3.376-1.559-6.642-.832-9.9.455a8.635 8.635 0 0 0-7.5.221c-1.222-.006-2.437-.006-3.653-.013-2.185-2.2-5-1.735-7.557-1.683-8.404.169-16.808.502-25.212 1-14.046.825-28.147-1.247-42.162.988-1.449.026-2.893.05-4.334.071a7.324 7.324 0 0 0-7.342.143c-1.455.117-2.9.24-4.359.357-3.855 1.813-7.839-2.034-11.682.279-.774-.026-1.547-.045-2.327-.065-1.7-1.949-3.4-2.579-5.108-.006-.657.078-1.308.156-1.964.24-4.484-3.441-9.291-1.341-14.005-.941a3.877 3.877 0 0 0-4.027.24l-9.1.234a6.532 6.532 0 0 0-5.66-1.1c-1.541-1.092-3.045-.682-4.537.149a6.056 6.056 0 0 0-5.562.286 4.05 4.05 0 0 0-4.549.11 75.885 75.885 0 0 0-20.228.559c-19.45-2.414-38.93-1.205-58.408-.406-1.921 0-3.843-.006-5.771-.006-3-1.2-5.906-.429-8.809.461-1.909.006-3.818.006-5.728.013a1.913 1.913 0 0 0-3.063.1c-1.393.032-2.8.065-4.193.1-9.755-2.293-19.571-1.319-29.387-.637l-4.119.117a6.566 6.566 0 0 0-6.028.481l-7.373-.253c-3.812-2.125-7.631-1.54-11.431.032-2.431-.039-4.862-.071-7.293-.11-3.468-.539-6.931-1.033-10.375.143-1.682-.942-3.315-.409-4.954.169-4.985-3.164-10.3-.871-15.47-1.072-.97-.143-1.94-.292-2.91-.435-3.818-2.228-7.8-.565-11.7-.526l-8.742-.1c-.933-1.39-1.811-1.338-2.658.084-3.413.013-6.876-.422-10.166.981-8.22.273-16.649-2.04-24.568 2.157-1.166.11-2.333.214-3.5.318-4.168-2.69-8.57-.91-12.892-.767l-2.769.156c-1.179-.793-2.155-.682-2.861.708-.945.247-1.9.487-2.848.734-8.146-.741-16.262-1.377-23.911 2.7a122 122 0 0 0-22.9.24c-1.08-.442-2.161-1.28-3.235-1.267-13.462.175-26.937.305-39.964 4.678-2.977-.2-5.961-.63-8.834.65-8.815-1.793-17.655-1.26-26.5-.331a8.714 8.714 0 0 0-7.017.273c-3.475-.214-6.968-.858-10.393.383-5.279-.448-10.583-1.241-15.8.6-2.726-.533-5.046.9-7.5 1.709-4.819-1.91-9.669-1.28-14.525-.188-.915-1.117-1.829-1.286-2.769-.045-4.3.78-8.809-.572-12.99 1.423l-10.454.39a14.728 14.728 0 0 0-9.914.39 2 2 0 0 0-1.663-.25 2.065 2.065 0 0 0-1.28 1.108"></path>
</svg>

  </figure>
</section>
              
<section
  data-block="signpost"
  class="relative overflow-hidden bg-white py-16 lg:py-32 ">
  <div class="container grid grid-cols-1 gap-8 lg:gap-x-0 items-center md:grid-cols-2">
    <div class="relative order-2  pt-12 mb:pb-12 md:mb-24  ">
      <div
        class="space-y-8 md:pl-16"
        data-sal="slide-left">
        
                  <h3 class="h2 text-line">Our Mission</h3>
                          <div class="mt-4 prose text-base font-normal"><p>At Midas Touch School of Excellence, our mission is to provide a nurturing and dynamic learning environment that empowers students to become lifelong learners, critical thinkers, and responsible global citizens. We are committed to fostering academic excellence, creativity, and holistic development by identifying and nurturing each child’s unique talents and abilities. Through collaborative efforts with parents and our academic partners, we strive to cultivate a culture of integrity, innovation, and respect.</p></div>
                <div class="mt-8 flex flex-wrap lg:flex-nowrap items-center gap-4">
                                <a class="c-button" href="#">Find out more</a>
                  </div>
      </div>
          </div>

    <div class="order-1 gs-animate" data-anim-x="-150" >
      <div class="w-[110%] md:w-[125%] -translate-x-[5%] md:-translate-x-1/4">
        
<div class="-rotate-6  u-mask-border">
      <div class="relative u-mask-image" x-data="{playing: true}">
      <span class="absolute inset-0 flex items-center justify-center h2-sm">Loading...</span>
      <video
      x-ref="video"
      data-src="<?= getMediaPath('mission_video_3') ?>" data-media-key="mission_video_3" muted loop role="presentation"
      class="w-full h-full object-cover aspect-video relative js-video-scroll"></video>
      <button
      class="absolute bottom-4 right-3 w-8 h-8 rounded-full border-2 border-white text-white flex items-center justify-center"
      @click="playing ? $refs.video.pause() : $refs.video.play(); playing = !playing"
      aria-label="Pause video">
      <i x-show="playing" class="block w-2.5 fill-current"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 9.571 13.713" class="gjqziaviya-fill-current">
  <path d="M0 0h2.571v13.712H0z" data-name="Rectangle 77"></path>
  <path d="M7 0h2.571v13.712H7z" data-name="Rectangle 78"></path>
</svg>
</i>
      <i x-show="!playing" class="block w-2.5 ml-1 fill-current"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 14">
  <path d="M12 7 0 14V0Z" data-name="Polygon 2"></path>
</svg>
</i>
    </button>
  </div>
    </div>      </div>
    </div>
  </div>
</section>
              <section
data-block="grid"
class="relative overflow-hidden container space-y-12 bg-white pt-16 lg:pt-32 mb-16 lg:mb-32">

    
    <div class="grid grid-cols-1 md:grid-flow-dense items-center gap-y-16 md:grid-cols-2">
                      <div
        class="relative "
        data-sal="slide-right">
          
<div class="-rotate-2  ">
          <figure role="presentation" class="absolute inset-px">
      

  <picture>
          <source
        srcset=
          "<?= getMediaPath('mission_leaf_1') ?>" data-media-key="mission_leaf_1"
        media="(min-width: 1024px)"
        type="image/webp">
      <source
        srcset=
          "<?= getMediaPath('mission_leaf_1') ?>" data-media-key="mission_leaf_1"
        media="(min-width: 768px)"
        type="image/webp">
      <source
        srcset=
          "<?= getMediaPath('mission_leaf_1') ?>" data-media-key="mission_leaf_1"
        type="image/webp">
        <source
      srcset=
        "<?= getMediaPath('mission_leaf_1') ?>" data-media-key="mission_leaf_1"
      media="(min-width: 1024px)">
    <source
      srcset=
        "<?= getMediaPath('mission_leaf_1') ?>" data-media-key="mission_leaf_1"
      media="(min-width: 768px)">
    <img
      src=
        "<?= getMediaPath('mission_leaf_1') ?>" data-media-key="mission_leaf_1"
      class="w-full h-full object-cover "
      alt="Pre-school children playing outside at midas touch"
      width="420"
      height="236"
      loading="lazy">
  </picture>
  </figure>
              <svg class="relative text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 902 583">
  <path fill="currentColor" fill-rule="evenodd" d="M0 0h902v583H0V0Zm595.181 35.748c-34.348-25.002-101.988.459-163.457 28.458l-.013-.074c-30.734 13.993-40.519 9.435-43.4 7.813-4.632-3.553-3.452-12.37-3.452-12.37 2.21-7.282 2.322-24.285-10.158-34.154-16.094-11.889-43.389-7.331-85.994 14.203-46.804 23.913-75.887 19.876-98.711 16.52-19.111-3.17-37.515-5.932-53.012 14.737L98.762 49.383c-4.322-2.428-11.102-.545-13.536 3.764-2.434 4.31-1.279 10.663 3.775 13.498l37.502 21.102c-9.313 23.194 2.161 38.191 14.778 52.865 14.677 17.746 33.193 40.482 36.868 92.803 2.832 47.106 13.35 73.88 32.287 80.74 14.927 5.548 29.456-3.343 35.689-9.312 1.528-1.028 7.675-5.152 12.406-3.443 2.571.496 11.536 6.49 15.734 41.089 7.761 66.946 21.296 137.136 60.525 153.507 17.807 7.17 38.433 2.662 60.823-15.083l8.084-5.883c9.623-6.922 17.695-12.804 22.712-16.618.216.751.426 1.502.633 2.254 4.396 15.751 13.3 45.41 51.907 81.347 19.82 18.736 35.789 25.82 50.355 23.591 17.957-3.17 27.009-20.83 35.64-37.783 6.606-13.349 13.61-27.417 23.767-30.24 6.358-1.164 14.07.322 24.166 6.006 21.644 12.173 36.658 15.888 49.473 11.715 13.933-4.483 19.409-17.511 23.259-27.677 1.713-4.719 4.246-10.873 6.507-11.505.397-.718 4.098-.532 10.592 3.121 44.717 25.151 82.555-4.756 96.922-25.114l.82-1.437c9.226-21.361 15.336-69.125-29.381-94.276-5.774-3.244-7.848-6.303-8.158-7.43-.634-2.241 3.34-7.59 6.506-11.504 6.321-7.814 15.088-19.95 11.723-34.167-3.465-12.371-14.728-24.395-35.64-36.16-10.095-5.683-16.491-11.181-17.658-17.522-2.819-10.13 5.625-23.38 13.672-35.925 10.071-16.136 20.565-32.989 13.995-49.955-5.613-13.585-19.943-23.541-46.281-30.773-50.814-14.365-80.866-6.613-96.662-2.229-.753.215-1.506.425-2.26.632-.037-6.663.832-16.595 2.521-27.975l.869-9.944c3.639-28.284-3.141-48.222-18.515-59.714Z" clip-rule="evenodd"></path>
</svg>

      </div>        </div>
        <div
        class="space-y-8 md:pl-16"
        data-sal="slide-left">
          <div class="prose text-base"><h2 class="text-base">Our Vision</h2>
<p class="text-base">Our vision is to be a leading institution recognized for delivering transformative education that prepares students to excel in a rapidly changing world. We aim to inspire confidence, pride, and enthusiasm in our students by promoting high standards in academics, extracurricular activities, sports, and personal growth. With a focus on continuous improvement and innovation in teaching, we aspire to shape future leaders who contribute positively to society.</p></div>
                      <div class="mt-8 flex flex-wrap lg:flex-nowrap items-center gap-4">
                                <a class="c-button" href="#">Find out more</a>
                  </div>
                  </div>
          </div>

</section>
          </div>
  </article>
      </main>
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	 
	   <?php require_once("includes/enquiry.php"); ?>
<?php require_once("includes/footer.php"); ?>
      <div class="c-button-chat origin-right animate-scale-bounce js-chat-btn hidden" style="--delay: 2s; --duration: 2s;">
        <a href="javascript:void(Tawk_API.toggle())"><span class="sr-only">Live Chat</span></a>
      </div>
      <div class="c-button-visit origin-right animate-scale-bounce" style="--delay: 1.5s; --duration: 2s;">
        <a href="#">
          <span class="sr-only">Book a visit</span>
        </a>
      </div>

 <div
  class="fixed inset-0 bg-white bg-opacity-90 p-10 md:px-24 flex items-center justify-center z-50"
  x-data="{open: false, content: ''}"
  @open-lightbox.window="open = true; content = $event.detail"
  x-show="open" x-cloak
  x-transition.opacity.duration.300ms>
  <div x-html="content" class="w-full"></div>
  <button
    class="appearance-none absolute right-5 top-5"
    @click="open = false; content = ''">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10">
      <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
    </svg>
  </button>
</div>
    
<?php require_once("includes/scripts.php"); ?>